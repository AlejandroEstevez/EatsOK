...
class TestEstablishmentOrder:
    ...
    def test_order_by_rating(
        ...
        response = authenticated_client.get(
            "/api/search/establishments/?ordering=-rating"
        )
    ...
    def test_order_by_distance(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data[0]["name"] == "Pizza Roma"
        assert response.data[0]["distance"] <= (
            response.data[1]["distance"]
        )
    ...
    def test_order_by_compatible_dishes(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data[0]["compatible_dishes"] >= (
            response.data[1]["compatible_dishes"]
        )
    ...
    def test_order_by_compatible_percentage(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data[0]["compatible_percentage"] >= (
            response.data[1]["compatible_percentage"]
        )
    ...
    def test_invalid_ordering_returns_bad_request(
        ...
        response = authenticated_client.get(
            "/api/search/establishments/?ordering=invalid"
        )
