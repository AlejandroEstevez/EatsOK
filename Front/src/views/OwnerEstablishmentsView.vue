...
async function loadEstablishments() {
  ...
  try {
    ...
    console.error('Error loading owner establishments:', err)

    establishments.value = []

    error.value = 'No se han podido cargar tus establecimientos.'
  } finally {
    loading.value = false
  }
}
...
function openCreateEstablishment() {
  router.push('/owner/establishments/new')
}
      ...
      <section class="owner-establishments-card">
        <header class="owner-establishments-header">
          <div>
            ...
            <p>Consulta y gestiona los establecimientos asociados a tu cuenta.</p>
          </div>
        ...
        <p v-if="loading" class="owner-establishments-state">Cargando establecimientos...</p>

        <p v-else-if="error" class="owner-establishments-state owner-establishments-state--error">
          {{ error }}
        </p>

        <div v-else-if="establishments.length" class="owner-establishments-list">
          <button
            v-for="establishment in establishments"
            :key="establishment.id"
            type="button"
            class="owner-establishment-item"
            @click="openEstablishment(establishment)"
          >
            ...
            <div class="owner-establishment-info">
              <div class="owner-establishment-title">
                ...
                <span
                  ...
                  :class="{
                    'owner-establishment-status--inactive': !establishment.active,
                  }"
                >
              ...
              <p v-if="establishment.description" class="owner-establishment-description">
                {{ establishment.description }}
              </p>
            </div>

            <span class="owner-establishment-arrow"> › </span>
          </button>
        </div>

        <div v-else class="owner-establishments-empty">
          <strong> Todavía no tienes establecimientos </strong>

          <p>Crea tu primer establecimiento para comenzar a gestionarlo desde EatsOK.</p>
        </div>
      </section>

      <p class="owner-establishments-slogan">Everyone can tag along</p>
    </main>
  </div>
</template>
