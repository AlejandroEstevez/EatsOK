...
const allergies = computed(() =>
  restrictions.value.filter(
    restriction => restriction.type === 'allergy',
  )
)
...
const selectedRestrictions = computed(() =>
  restrictions.value.filter(
    restriction =>
      selectedRestrictionIds.value.includes(restriction.id),
  )
)
...
const loadFoodProfile = async () => {
  ...
  try {
    ...
    selectedRestrictionIds.value =
      profileResponse.data.restrictions.map(
...
const toggleRestriction = (restrictionId) => {
  if (
    selectedRestrictionIds.value.includes(restrictionId)
  ) {
    selectedRestrictionIds.value =
      selectedRestrictionIds.value.filter(
        id => id !== restrictionId,
      )
  ...
  >
    ...
    <main class="food-profile-main">
      <header class="food-profile-title">
        ...
        <p class="food-profile-description">
          Selecciona tus alergias, intolerancias y preferencias alimentarias
          para adaptar establecimientos, platos y recetas a tus necesidades.
        </p>
      </header>
      ...
      <template v-else>
        <section class="profile-summary">
          <div class="summary-content">
            ...
            <div
              v-if="selectedRestrictions.length"
              class="summary-restrictions"
            >
              <RestrictionCard
          ...
          <section class="restrictions-section">
            ...
            <div class="allergies-grid">
              <RestrictionCard
                ...
                :restriction="restriction"
                :selected="
                  selectedRestrictionIds.includes(
                    restriction.id,
                  )
                "
                @toggle="toggleRestriction"
    ...
    >
      <div class="profile-modal">
        ...
        <p>
          Tus preferencias alimentarias se han
