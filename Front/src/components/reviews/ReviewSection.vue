...
const props = defineProps({
  targetType: {
    type: String,
    required: true,
    validator: (value) => ['establishment', 'recipe'].includes(value),
  },
...
const formattedAverageRating = computed(() => {
  return averageRating.value.toFixed(1).replace('.', ',')
})
...
async function submitReview({ rating, comment }) {
  ...
  try {
    ...
    submitError.value = parseReviewError(error)
  } finally {
...
async function hideReview(review) {
  const confirmed = window.confirm('¿Seguro que quieres ocultar esta reseña?')
  ...
  try {
    await api.patch(`/reviews/${review.id}/moderation/`, {
      visible: false,
    })
...
<template>
  <section class="review-section">
    ...
    <p v-if="loading" class="reviews-state">Cargando reseñas...</p>

    <p v-else-if="!reviews.length" class="reviews-state">Aún no hay reseñas.</p>
        ...
        <div class="review-content">
          <div class="review-header">
            ...
            <span class="review-date">
              {{ formatRelativeDate(review.publication_date) }}
            </span>
            ...
            <button
              ...
              :disabled="hidingReviewId === review.id"
              @click="hideReview(review)"
            >
              {{ hidingReviewId === review.id ? 'Ocultando...' : 'Ocultar' }}
            </button>
          </div>
